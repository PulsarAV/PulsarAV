...
class AccountMove(models.Model):
    ...
    def _compute_export_reset_to_draft_button(self):
        """compute show button to reset move to draft"""
        for move in self:
    ...
    def _get_default_accounts(self):
        ...
        if self.payment_id[:1].payment_type == "customer":
            return [
                self.env.company.account_journal_payment_debit_account_id.id,
            ]
        if self.payment_id[:1].payment_type == "supplier":
            return [
                self.env.company.account_journal_payment_credit_account_id.id,
            ]
        return [
    ...
    def _compute_export_datev_account(self):  # noqa: C901
        ...
        for move in self:
            ...
            payment_account = move.line_ids.filtered(
                lambda line: line.account_id.id in self._get_default_accounts()
            )
