...
class SyscoonFinanceinterface(models.Model):
    ...
    def export(self, mode=False, date_from=False, date_to=False, args=False):
        ...
        if kind == "rewe":
            export_header = self.generate_partner_export_header(
                self.header_template(), date_from
            )
    ...
    def _prepare_partner_invoice_address_values(self, partner_id):
        values = {}
        child_id = partner_id.child_ids.filtered(lambda c: c.type == "invoice")
        if child_id:
    ...
    def _prepare_debitor_payment_term_values(self, partner_id):
        ...
        for line in partner_id.property_payment_term_id.line_ids:
            if line.value == "balance":
                values["Fälligkeit in Tagen (Debitor)"] = line.days
            if line.value == "percent" and not values["Skonto in Prozent (Debitor)"]:
