...
class SyscoonFinanceinterface(models.Model):
    ...
    def generate_export_lines(self, move):
        ...
        if export_lines and float_repr(amount_check, 2) != float_repr(
            ...
            converted_line = self.correct_amount(
                export_lines, amount_check, amount_real
            )
        export_lines = self.group_converted_move_lines(
            export_lines, move.journal_id.datev_ascii_group_moves, group
        )
        return export_lines
    ...
    def generate_export_line(self, export_line, line):  # noqa: C901
        ...
        if (
            ...
            export_line["Kurs"] = line.amount_currency / line.balance
    ...
    def create_doc_field(self, line, group):
        ...
        if line.move_id.has_reconciled_entries:
            for ml in self.env["account.move.line"].browse(
                line.move_id.line_ids._reconciled_lines()
            ):
                if (
                    ml.move_id.move_type
                ...
                if not doc_field and ml.move_id.has_reconciled_entries:
                    for ml2 in self.env["account.move.line"].browse(
                        ml.move_id.line_ids._reconciled_lines()
                    ):
                        if (
                            ml2.move_id.move_type
                            in ["out_invoice", "out_refund", "in_invoice", "in_refund"]
                            and ml2.move_id.datev_ref
                        ):
                            doc_field = ml2.move_id.datev_ref
        if (
            self.env["ir.module.module"]
            .sudo()
            .search([("name", "=", "account_asset"), ("state", "=", "installed")])
        ):
            move_lines = (
                line.move_id.asset_id.original_move_line_ids
                or line.move_id.reversed_entry_id.asset_id.original_move_line_ids
            )
            doc_field = (
                move_lines[:1].move_id.datev_ref
                or line.move_id.reversed_entry_id.datev_ref
            )
        if not doc_field:
